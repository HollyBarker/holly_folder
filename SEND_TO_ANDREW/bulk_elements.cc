	 ...
	 if(local_unknown >= 0)
	 {
	  ...
	  else
	  {
	   ...
	   if(Reaction_deriv_fct_pt==0)
	   {
	    ...
	    //This is adding the dRdC part only
	    jacobian(local_eqn,local_unknown) -= 
	     dRdC_FD(r,r2)*psi(l2)*test(l)*W;
	    
	    for(unsigned i=0;i<DIM;i++)
	    {
	     //This is adding the dRddCdx part only
	     jacobian(local_eqn,local_unknown) -=
	  ...
	  if(F_fct_pt==0)
	  {
	   ...
	   dFdC_FD(r,r2)=0.0;
	   //Set the derivatives of F with respect to the gradient of
	  ...
	  else
	  {
	   ...
	   if (F_deriv_fct_pt==0)
	   {
	    ...
	    for(unsigned k=0;k<DIM;k++)
	    {
	     //This is adding the dFdC part only 
	     jacobian(local_eqn,local_unknown) -=
	     ...
	     for(unsigned i=0;i<DIM;i++)
	     {
	      //This is adding dFddCdx part only 
	      jacobian(local_eqn,local_unknown)-=

...
//3 corresponds to the displacement
//4 corresponds to the volume fraction equation
...
{ 
 //Vector to hold the timescale parameters (multiplying the
 //time derivatives)
 //These are set in the function activator_inhibitor_tau
 Vector<double> Tau(5,1.0);

 //Vector to hold the diffusion parameters (multiplying the second
 //spatial derivatives)
 //These are set in the function activator_inhibitor_diff
 Vector<double> D(5,1.0);

 //====================================================================
 //====================================================================
 //This is where lots of constants are defined - for the most part they
 //are particular to the problem
 //====================================================================
 //====================================================================

 //Timestep size is needed for the finite difference calculation
 //of df/dt
 ...
 //Function to set the 'diffusivities' D(C)
				   ...
				   Vector<double> &R)
 {
  ...
  //includes df/dt, so I've included the equation from the finite
  //difference calculation here.
  double dfdt_FD;
  double tempdfdt_FD=((1-C[4])*(C[0]-CTSS)*a*a)/
   ((char_time+timestep)*(diffusivity)*(Cb-CTSS));
  
  //This 'if' statement controls chemistry in the equations: if there is
  //a high enough concentration, reaction occurs. If there is a low
  //enough concentration, dissociation occurs (provided there is any
  // product to dissociate)
 ...
 */
 ...
 //Flux on the rightmost boundary 
...
//========================================================================
 ...
 //The problem is not done on a refineable mesh as the 1D refineable ADR
 //mesh is not working at present
 //'Refineable' is just the name of the Problem class object
...
RefineableOneDAdvectionDiffusionReactionProblem()
{
 ...
 //To obtain output files of .dat type, the extension type should be changed
 //below, and output_paraview(...) should be exchanged for output(...)
 ...
 // The ID of the leftmost boundary (INNER)
 unsigned left_boundary_id=0;
 
 // The ID of the rightmost boundary (OUTER)
 ...
 // free by default
 // Here the nodes with Dirichlet BCs are pinned
...
create_flux_elements(const unsigned &b, Mesh* const &bulk_mesh_pt,
                     Mesh* const &surface_mesh_pt)
{
 // Get the number of bulk elements adjacent to boundary b
 ...
 const double &dt, const unsigned &nstep)
{
 ...
 //(Spatial adaptivity does not work at present)
 ...
 //Zero rounds of adaptation per timestep since adaptable mesh does not work
 max_adapt = 0;
